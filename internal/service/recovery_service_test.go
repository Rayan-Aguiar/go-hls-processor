...
func TestRecoveryServiceRecover_RequeuesStalePendingJobs(t *testing.T) {
	conn := newRecoveryTestDB(t)

	oldTime := time.Now().Add(-10 * time.Minute)
	insertJobForRecovery(t, conn, "job-pending-stale", "pending", oldTime, oldTime)
	...
	if adapter.enqueued[0].JobID != "job-pending-stale" {
		t.Fatalf("expected job-pending-stale, got %s", adapter.enqueued[0].JobID)
	}
}

func TestRecoveryServiceRecover_DoesNotRequeueFreshPendingJobs(t *testing.T) {
	conn := newRecoveryTestDB(t)
